    ...
    def close(self):
        ...
        if not self._closed:
            ...
            if self.file_obj is not None:
                ...
                total_receive = total_receive + self.tot_bytes_received
                total_send = total_send + self.tot_bytes_sent
                logger.info("Total sent:%s ,Total received:%s",total_send,total_receive)
...
class FTPHandler(AsyncChat):
    ...
    def handle_whitelist(self):
        """Called when IP is not in white list."""
        msg = "421 IP is not in white list."
        self.respond_w_warning(msg)
        self.close_when_done()

    def handle_blacklist(self):
        """Called when IP is in blacklist."""
        msg = "421 IP is in black list."
        self.respond_w_warning(msg)
        self.close_when_done()
